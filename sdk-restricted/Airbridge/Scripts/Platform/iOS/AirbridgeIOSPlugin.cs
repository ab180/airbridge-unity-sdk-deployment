...
internal class AirbridgeIOSPlugin : IAirbridgePlugin
{
    ...
    private static int ToiOSTrackingBlocklistValue(AirbridgeTrackingBlocklist item)
    {
        switch (item)
        {
            case AirbridgeTrackingBlocklist.IDFA: return 0;
            case AirbridgeTrackingBlocklist.IDFV: return 1;
            case AirbridgeTrackingBlocklist.SKAdNetwork: return 2;
        }

        return -1;
    }
    
    [DllImport("__Internal")] private static extern void native_blockTrackingItem(int item);
    public void BlockTrackingItem(AirbridgeTrackingBlocklist item)
    {
        native_blockTrackingItem(ToiOSTrackingBlocklistValue(item));
    }
    
    [DllImport("__Internal")] private static extern void native_allowTrackingItem(int item);
    public void AllowTrackingItem(AirbridgeTrackingBlocklist item)
    {
        native_allowTrackingItem(ToiOSTrackingBlocklistValue(item));
    }
    #endregion
