...
public class Airbridge
{
   ...
   private static IAirbridgePlugin AirbridgePlugin => Lazy.Value;
    ...
    /// <summary>
    /// Blocks tracking for the specified <see cref="AirbridgeTrackingBlocklist"/> item.
    /// This function adds the given item to the current TrackingBlocklist
    /// applied in the <c>Configure SDK settings</c>, disabling tracking at runtime.
    /// </summary>
    /// <param name="item">The tracking blocklist item to block.</param>
    public static void BlockTrackingItem(AirbridgeTrackingBlocklist item)
    {
        AirbridgePlugin.BlockTrackingItem(item);
    }

    /// <summary>
    /// Allows tracking for the specified <see cref="AirbridgeTrackingBlocklist"/> item.
    /// This function removes the given item from the current TrackingBlocklist
    /// applied in the <c>Configure SDK settings</c>, enabling tracking at runtime.
    /// </summary>
    /// <param name="item">The tracking blocklist item to allow.</param>
    public static void AllowTrackingItem(AirbridgeTrackingBlocklist item)
    {
        AirbridgePlugin.AllowTrackingItem(item);
    }
